...
from math import *
import tqdm
from config import FEWSHOTSAVEPATH, MVTEC_LIST
from utils import get_dataset_categories

random.seed(1003)

DATASETROOT = FEWSHOTSAVEPATH
...
def make_data(
    ...
    good = os.path.join(dataRoot, obj, "train", "good")
    ...
    with open(annotaion_path, "w") as f:
        ...
        while True:
            for index in range( start_num,GENERATENUM):
                ...
                m_item = os.path.join(
                    mask_anomaly, os.path.basename(a_item)[:-4] + "_mask.png"
                )
...
if __name__ == "__main__":
    augment_control_data_save_path =  "/your/path/mvtec_triple_dataset"
    for obj in MVTEC_LIST:
        obj_path = os.path.join(DATASETROOT, obj, "fewshot")
        categories = get_dataset_categories(obj_path)
