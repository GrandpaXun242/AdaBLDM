...
from math import *
import tqdm
from config import FEW_SAVE_PATH, file_suffix

random.seed(1003)

DATASETROOT = FEW_SAVE_PATH
...
def make_data(
    ...
    real_anomaly = os.path.join(dataRoot, obj, "fewshot", cate)
    mask_anomaly = mask_path
    good = os.path.join(dataRoot, obj, "train", "ok")
    good_foreground_suffix = os.path.join(dataRoot, obj, "foregound", fore_mode, "ok")
    real_anomaly_list = glob.glob(os.path.join(real_anomaly, "*.*"))

    real_good_list = glob.glob(os.path.join(good, "*.*"))
    ...
    with open(annotaion_path, "w") as f:
        ...
        while True:
            for index in range( start_num,GENERATENUM):
                ...
                m_item = os.path.join(
                    mask_anomaly, os.path.basename(a_item)[:-4] + ".png"
                )
...
if __name__ == "__main__":
    augment_control_data_save_path = "/home/your/path/btad_triple_dataset"
    for obj in ["01" ,"02" ,"03" ]:

        obj_path = os.path.join(DATASETROOT, obj, "fewshot")
        categories = ["ko"]
        for cate in categories:
            generate_num = glob.glob(
                os.path.join(
                    augment_control_data_save_path, obj, cate, "source", "*.*g"
                )
            )
            generate_num = len(generate_num)

            if generate_num > GENERATENUM:
                continue
            mask_path = os.path.join(DATASETROOT, obj, "ground_truth", cate)
            print("Now is to genrate ", obj, cate)
            make_data(
                dataRoot  = DATASETROOT,
                save_path = os.path.join(augment_control_data_save_path, obj, cate),
                obj       = obj,
                cate      = cate,
                mask_path = mask_path,
                start_num = generate_num,
                ishow     = False
            )
