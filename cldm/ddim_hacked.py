...
class DDIMSampler(object):
    ...
    def sample_blended(
        ...
        **kwargs,
        ...
        samples, intermediates = self.ddim_sampling_with_pixel_latent_blend(
            conditioning,
            size,
            callback=callback,
    ...
    @torch.no_grad()
    def ddim_sampling_with_pixel_latent_blend(
        self,
        cond,
        shape,
        x_T=None,
        ...
        ucg_schedule=None,
        init_image=None,
        percentage_of_pixel_blending=0.5,
        cutoff_index=0,
    ):
        cutoff_point = cutoff_index
        ...
        for i, step in enumerate(iterator):
            ...
            if mask is not None and i <= cutoff_point and i > cutoff_point - 20:
            ...
            if org_mask is not None and i >= cutoff_point:
                ...
                pixel_blended = (
                    foreground_pixels * (1 - org_mask) + background_pixels * org_mask
                )
    ...
    def ddim_sampling(
        ...
        for i, step in enumerate(iterator):
            index = total_steps - i - 1
            ts = torch.full((b,), step, device=device, dtype=torch.long)
            # if mask is not None and index < 20 and index > 3:
            if mask is not None and index < 20:
